package creeps;

public class Reference {
    public static final String MOD_ID = "creeps";
    public static final String MOD_NAME = "More Creeps And Weirdos Reboot";
    public static final String MOD_VERSION = "BETA";
    public static final String MOD_BUILD_NUMBER = "0.4";
    public static final String MOD_CHANNEL = MOD_ID;
    public static final String MOD_MC_VERSION = "1.8";

    public static final String MODEL_PATH = "models/";
    public static final String TEXTURE_PATH_MODELS = "textures/models/";
    public static final String TEXTURE_PATH_ENTITIES = "textures/entities/";
    public static final String TEXTURE_PATH_ITEMS = "textures/items/";
    public static final String CONFIG_PATH = "assets/mocreeps/config/";

    public static final String MUMMY_TEXTURE_NAME = "mummy.png";
    public static final String GROWBOTGREGG_TEXTURE_NAME = "growbotgregg.png";
}
